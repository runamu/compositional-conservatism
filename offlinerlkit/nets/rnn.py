...
class LSTM(nn.Module):
    def __init__(
        ...
        self.input_dim=input_dim # input_dim = [batch_size,time_steps,features]
        self.hidden_dims=hidden_dims
        self.num_layers=num_layers
        self.device = device
        self.output_dim = hidden_dims
        # h0 = torch.zeros(self.num_layers, batch_size, self.hidden_dims).to(self.device)
        # c0 = torch.zeros(self.num_layers, batch_size, self.hidden_dims).to(self.device)
        self.lstm = nn.LSTM(
                            input_size = input_dim,
        ...
        # # Initial states as trainable parameters
        # self.fc = nn.Linear(hidden_dims, hidden_dims)

    def forward(self, x, h0=None, c0=None):
        batch_size, time_step, obs_sim = x.shape
        #self.lstm.flatten_parameters()
        if h0 is None:
            h0 = torch.zeros(self.num_layers, batch_size, self.hidden_dims, device=self.device)
            nn.init.xavier_uniform_(h0)

        if c0 is None:
            c0 = torch.zeros(self.num_layers, batch_size, self.hidden_dims, device=self.device)
