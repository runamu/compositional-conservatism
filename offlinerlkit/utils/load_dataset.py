...
def qlearning_dataset(env, dataset=None, terminate_on_end=False, **kwargs):
    ...
    if dataset is None:
        ...
        raise ValueError("qlearning_dataset is implemented for the maze and ant maze environments for now. its gonna be updated soon")
...
def qlearning_dataset_d4rl(env, dataset=None, terminate_on_end=False, **kwargs):
    """
    Returns datasets formatted for use by standard Q-learning algorithms,
    with observations, actions, next_observations, rewards, and a terminal
    flag.
    ...
    dataset = {
        ...
        'rewards': np.array(reward_),
        'terminals': np.array(done_),
        'infos/step': np.array(step_),
    }
    return dataset

def qlearning_dataset_neorl(env, dataset=None, terminate_on_end=False, **kwargs):
    """
    Returns datasets formatted for use by standard Q-learning algorithms,
    with observations, actions, next_observations, rewards, and a terminal
    flag.
    ...
    """
    if dataset is None:
        dataset, _ = env.get_dataset(**kwargs)
    else:
        pass
    ...
    episode_step = 0
    for i in range(N-1):
        obs = dataset['obs'][i, 1:].astype(np.float32)
        new_obs = dataset['next_obs'][i+1, 1:].astype(np.float32)
...
# this is from https://github.com/yihaosun1124/mobile
...
def normalize_reward(rewards: List):
    eps = 1e-3
    mean = sum(rewards) / len(rewards)
    std_deviation = (sum((x - mean) ** 2 for x in rewards) / len(rewards)) ** 0.5
    normalized_rewards = [(x - mean) / (std_deviation + eps) for x in rewards]
