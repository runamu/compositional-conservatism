...
def get_args(argv=None):
    ...
    parser.add_argument("--penalty_coef", '-pc',type=float, default=1.5)
    parser.add_argument("--num_samples", type=int, default=10)
    parser.add_argument("--num_q_ensemble", "-nqe", type=int, default=2)
    parser.add_argument("--max_q_backup", "-mqb", type=lambda x: bool(strtobool(x)), default=False)
    parser.add_argument("--norm_reward", "-nr", type=lambda x: bool(strtobool(x)), default=False)
    ...
    parser.add_argument("--lr_scheduler", type=lambda x: bool(strtobool(x)), default=True, nargs="?", const=True)
...
def train(args):
    ...
    critic_backbone = MLP(input_dim=critic_backbone_input, hidden_dims=args.hidden_dims, layernorm=args.layernorm)
    ...
    if args.load_anchor_seeker_path:
        ...
        for obj in policy.critics + policy.critics_old:
    ...
    log_dirs = make_log_dirs(
        ...
        record_params=["penalty_coef", "rollout_length", "real_ratio"],
        timestamp=timestamp,
        init_run=True,
    )
